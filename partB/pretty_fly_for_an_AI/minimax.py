...
def max_value_ml(state, depth, ev, alpha, beta):
    if depth == 0 or st.is_gameover(state):
...
def min_value_ml(state, depth, ev, alpha, beta):
    if depth == 0 or st.is_gameover(state):
        return ev(state), None, state
    ...
    move = None
    for mv, child_state in st.next_states(state, opponent=True):
        score, came_from, leaf_state = max_value_ml(child_state, depth - 1, ev, alpha, beta)
        if score < v:
            v = score
            pred_state = leaf_state
            move = mv
